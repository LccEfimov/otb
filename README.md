...
После `python scripts/seed.py` создаются/обновляются (идемпотентно):
- роли `admin` и `user`;
- минимум 17 пользователей: `admin` + `user01`..`user16`;
- банк из 160 вопросов в 8 категориях, у каждого вопроса 4 варианта ответа и 1 корректный;
- демо-назначение `Демо-тестирование` для `user01`.

Правило идемпотентности:
- пользователи обновляются по `username`;
- вопросы обновляются по точному тексту вопроса;
- варианты ответов для существующего вопроса синхронизируются с seed-шаблоном.
