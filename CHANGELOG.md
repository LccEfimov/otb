...
- improved release bundling to auto-detect `TerraTesting.exe` / packaged outputs in `dist/` before fallback to build files
- CI lint workflow now runs blocking Ruff checks for maintained release/test scripts (no `|| true`)
- README quick start now documents copy command variants for Windows CMD/PowerShell and Linux/macOS
